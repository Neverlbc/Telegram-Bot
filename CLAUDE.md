...
| `create_ae_promo_code` | 创建速卖通折扣码（通常 5-15 分钟生效） | `store_name`, `discount_value`, `min_order_amount`, `validity_days`, `total_num`, `num_per_buyer` |

创建折扣码时：若用户未指定店铺 → 先调 `list_ae_stores` 展示可选列表 → 用户选择 → 收集缺少的参数 → 调用创建。
